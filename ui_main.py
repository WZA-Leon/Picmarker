...
class PhotoWatermarkApp:
    ...
    def _init_ui(self):
        ...
        def on_mousewheel_left(event):
            ...
            if self._mouse_in_left:
                ...
                if bbox:
                    ...
                    content_h = bbox[3]
                    print(f"[DBG] mouse_in_left={self._mouse_in_left} canvas_h={canvas_h} content_h={content_h} yview={self.left_canvas.yview()}")
                    # 内容高度不超过可视高度时禁止滚动
        ...
        self.cbo_f = ttk.Combobox(row5, textvariable=self.f_var, width=20, state="readonly",
                                  values=GUI_CFG.get("aperture_values", []))
        ...
        self.cbo_exp = ttk.Combobox(row6, textvariable=self.exp_var, width=20, state="readonly",
                                    values=GUI_CFG.get("shutter_values", []))
