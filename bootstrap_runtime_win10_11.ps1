...
$RUNTIME = Join-Path $WORK_DIR 'runtime_win10_11'
$PY_ZIP = Join-Path $RUNTIME 'python_embed.zip'
$PACKAGES = @('numpy','pillow','piexif','exifread','PyWavelets')
...
$env:TCL_LIBRARY = Join-Path $RUNTIME 'tcl\tcl8.6'
$env:TK_LIBRARY = Join-Path $RUNTIME 'tcl\tk8.6'
Start-Process -FilePath $pyExe -ArgumentList (Join-Path $WORK_DIR 'main.py') -WorkingDirectory $WORK_DIR
