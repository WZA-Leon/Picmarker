...
class PhotoWatermarkApp:
    ...
    def _init_ui(self):
        ...
        ttk.Entry(row4, textvariable=self.photo_name_var, width=20, state="normal").pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Entry(row9, textvariable=self.time_var, width=20, state="normal").pack(side=tk.LEFT, padx=(0, 10))
        
        ttk.Label(row10, text="地点：", width=6).pack(side=tk.LEFT)
        ttk.Entry(row10, textvariable=self.loc_var, width=20, state="normal").pack(side=tk.LEFT, padx=(0, 10))
        
        ttk.Label(row11, text="字体：", width=6).pack(side=tk.LEFT)
        self.font_cb = ttk.Combobox(row11, textvariable=self.selected_font,
        ...
        ttk.Label(panel_text.content, text="功能暂未开放", foreground="gray").pack(pady=20)

        # 3. 隐形水印面板
        panel_hidden = CollapsiblePanel(self.left_scroll_content, "隐形水印", expanded=False)
        panel_hidden.pack(fill="x", pady=3)
        ttk.Label(panel_hidden.content, text="功能暂未开放", foreground="gray").pack(pady=20)

        # 4. 图片调整面板
        panel_adjust = CollapsiblePanel(self.left_scroll_content, "图片调整", expanded=False)
        panel_adjust.pack(fill="x", pady=3)
        ttk.Label(panel_adjust.content, text="功能暂未开放", foreground="gray").pack(pady=20)

        # 5. 图片标注面板
        panel_annotate = CollapsiblePanel(self.left_scroll_content, "图片标注", expanded=False)
        panel_annotate.pack(fill="x", pady=3)
        ttk.Label(panel_annotate.content, text="功能暂未开放", foreground="gray").pack(pady=20)
    ...
    def auto_refresh_preview(self):
        self.show_preview()
        self.root.after(300,self.auto_refresh_preview)
